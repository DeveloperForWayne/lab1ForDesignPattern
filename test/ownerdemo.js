...
contract("OwnerDemo", accounts => {
  ...
  it("Should allow owner to call functions with the onlyOwner modifier");
  ...
  it("Should allow owner to transfer ownership", async function() {
    ...
    let result= await ownerDemoInstance.transferOwner(newOwner);
    ...
    assert.equal(result.logs[0].args.previousOwner,currentOwner,'previousOwner is not correct');
    assert.equal(result.logs[0].args.newOwner,newOwner,'newOwner is not correct');
  });
